...
- **TODO (optional): CI test pipeline** — GitHub Actions could run the integration tests against a real Foundry server. Pieces needed: Foundry server zip (stored as a private artifact or downloaded via the Foundry API using a license key secret), world config files (static JSON, commit to repo), Playwright (already available in GH Actions). Main value would be automatic regression checks on PRs. Not worth doing until the module is more mature / community-facing.
